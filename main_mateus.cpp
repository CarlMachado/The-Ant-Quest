...
// Coloca uma nova formiga no mapa
...
void desenharFrame(Mapa m, Controle c, Formiga f, Item it) {
	...
	al_draw_text(c.fonte[T10], al_map_rgb(102, 51, 0), 490, 45, NULL, "pa: ");
	al_draw_text(c.fonte[T10], al_map_rgb(102, 51, 0), 670, 45, NULL, qtdPa);
	/*----------------------------------------------------------------*/
}
...
void sorteiarmazens(Mapa &m) {
	bool testa = true, testai = true, testaii = true;
	int x, y;
	while (testa) {
		x = rand() % (m.x);
		y = rand() % (m.y);
			if (y < (m.y / 3) && x > 0 && x < (m.x/3)) {
				if (m.mapa[y][x] == PAREDE) {
					if (m.mapa[y + 1][x] == CAMINHO || m.mapa[y][x + 1] == CAMINHO) {
						testa = false;
						m.mapa[y][x] = ARMAZEM_MAPA_1;
						x = 0;
						y = 0;
					}
				}
	...
	while (testaii) {
		x = rand() % (m.x);
		y = rand() % (m.y);
		if (y > ((m.y * 2) / 3) && y<(m.y) && x >(((m.x * 2) / 3)) && x < (m.x)) {
			if (m.mapa[y][x] == PAREDE) {
				if (m.mapa[y - 1][x] == CAMINHO || m.mapa[y][x - 1] == CAMINHO) {
					testaii = false;
					m.mapa[y][x] = ARMAZEM_MAPA_3;
					x = 0;
					y = 0;
				}
			}
